  ...
  useEffect(() => {
    onAuthStateChanged(fireAuth, (currUser) => {
      if (currUser) {
        ...
        console.log(currUser.email + " is logged in");
        console.log(currUser.uid + currUser.id +  "Is logged in");
        setLoggedIn(true)
      }
      else {
      ...
      <BrowserRouter>
        ...
        <Routes>
          ...
          <Route path='/Watchlist' element={<WatchList tokenList={tokenList} addNewToken={addNewToken} WatchlistRef={WatchlistRef} removeFromWatch={removeFromWatch}
            newToken={newToken} setTokenList={setTokenList} isLoggedIn={isLoggedIn} loggedUser={loggedUser} setLoggedUser={setLoggedUser} />}>
          </Route>
