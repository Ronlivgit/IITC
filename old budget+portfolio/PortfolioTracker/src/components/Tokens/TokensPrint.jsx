    ...
    return (
        <>
            <div className="tokenInfoDiv" >
                ...
                <button onClick={()=>{props.addToWatch(props.trInfo,props.loggedUser),setIsMarked(!isMarked);}} itemToWatch={props.rankIndex}>Add to Watch list</button>
                <button ><Link to={`/tokens/${props.trInfo.id}`}>Press to highlight</Link></button>
            </div>
        </>
    )
}
