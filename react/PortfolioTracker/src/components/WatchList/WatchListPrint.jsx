    ...
    return (
        <>
            <div className="watchedToken" >
                ...
                <button onClick={(e)=>{props.removeFromWatch(props.watchedItem,props.loggedUser),props.watchedItemsID.splice(props.watchedItem.rank,1)}}
                itemIndex={props.watchedItem.rank} className="removerBTN">Press to remove</button>
                <button className="highlightBtn"><Link to={`/Watchlist/${props.watchedItem.id}`}>Press to highlight</Link></button>
            </div>
        </>
    )
}
